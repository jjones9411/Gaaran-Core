...
const InfoPanel2 = () => {
  ...
  useEffect(() => {
    const addImageClickListeners = () => {
      const images = document.querySelectorAll('.info2-content img');
      images.forEach(img => {
  ...
  const fetchInfos = async () => {
    try {
      const token = localStorage.getItem('token');
      const res = await axios.get('/api/infos2', { headers: { Authorization: `Bearer ${token}` } });
      ...
      console.error('Błąd pobierania infos2:', err);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.id) {
        await axios.put(`/api/infos2/${formData.id}`, data, { headers });
        fetchInfos();
      } else {
        const res = await axios.post('/api/infos2', data, { headers });
        fetchInfos();
      }
      ...
      console.error('Błąd zapisu infos2:', err.response?.data || err.message);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`/api/infos2/${id}`, { headers: { Authorization: `Bearer ${token}` } });
      setInfos(infos.filter((b) => b.id !== id));
    } catch (err) {
  ...
  return (
    <PanelPage>
      <PanelHeader eyebrow="// PORADNIKI" title="Informacje" theme={theme} />

      {userRole === 'admin' && !isEditing && (
        <Button sx={panelAddButtonSx(theme)} onClick={() => {
          setFormData({ title: '', content: '', id: null });
          setIsEditing(true);
        }}>Dodaj Informację</Button>
      )}
